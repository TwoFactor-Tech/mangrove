...
func (m *Mangrove) signRequest(request *http.Request) (string, error) {
	...
	for _, key := range []string{"content-type", "host", "x-guarder-id", "x-guarder-signed-at", "x-guarder-uuid"} {
		signaturePayload = append(
			signaturePayload, fmt.Sprintf("%s: %s", key, request.Header[key][0]),
		)
	}
	// > > 空行
			...
			pie.Join(
				pie.Map(
					...
					func(key string) string {
						return fmt.Sprintf("%s=%s", key, transformedRequestBody[key])
					},
				),
				"&",
			),
...
func (m *MangroveMerchantOrder) Create(merchantOrders []MerchantOrder) (merchantOrders0 []MerchantOrder, wrong error) {
	...
	for _, merchantOrder := range merchantOrders {
		if merchantOrder.Order.OrderCallback.Endpoint != "" {
			merchantOrder.Order.OrderCallback.Endpoint = fmt.Sprintf("v2:%s", merchantOrder.Order.OrderCallback.Endpoint)
		}
	}
	// -
	wrong = m.m.request(
